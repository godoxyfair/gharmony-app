...
function chordNotes(chord: Chord, octave = 3): string[] {
  return CHORD_SEMITONES[chord.quality].map(s => noteStr(chord.root + octave * 12 + s))
}

const waveCache = new WeakMap<BaseAudioContext, PeriodicWave>()

function getPianoWave(ctx: BaseAudioContext): PeriodicWave {
  const hit = waveCache.get(ctx)
  if (hit) return hit
  const real = new Float32Array(16)
  const imag = new Float32Array(16)
  imag[1] = 1.0; imag[2] = 0.45; imag[3] = 0.22; imag[4] = 0.10
  imag[5] = 0.05; imag[6] = 0.025; imag[7] = 0.012; imag[8] = 0.006
...
export function stopB() { cleanup() }

export async function playB(
  notes: QuantizedNote[],
  chords: ChordEvent[],
  bpm: number,
  style: StyleName,
  ...
  // ── melody: raw Web Audio PeriodicWave scheduled via Tone.js timing ──
  const nCtx = Tone.getContext().rawContext as unknown as AudioContext
  const wave = getPianoWave(nCtx)

  const melPart = new Tone.Part(
    (time: number, v: { midi: number; dur: number; clarity: number }) => {
      const freq = midiToFreq(v.midi)
      const vel = 0.28 * Math.max(0.5, v.clarity)
      const end = time + v.dur
      const rel = end + 0.12

      const master = nCtx.createGain()
      master.connect(nCtx.destination)
      master.gain.setValueAtTime(0, time)
      master.gain.linearRampToValueAtTime(vel, time + 0.004)
      master.gain.exponentialRampToValueAtTime(vel * 0.6, time + 0.05)
      master.gain.exponentialRampToValueAtTime(vel * 0.25, end)
      master.gain.exponentialRampToValueAtTime(0.0001, rel)

      const o1 = nCtx.createOscillator()
      const o2 = nCtx.createOscillator()
      ...
      o1.stop(rel + 0.05); o2.stop(rel + 0.05)
    },
    notes.map(note => ({
      time: note.startTime,
      midi: note.midi,
  ...
  // ── chords, bass, drums — same as Variant A ────────────────────────────
  ...
  parts.push(new Tone.Part(
    ...
    chords.map(c => ({
      time: c.beat * bd,
      nn: noteStr(c.chord.root + bassOct * 12),
  ...
  if (style === 'folk') {
    ...
    const hat = new Tone.MetalSynth({
      ...
      harmonicity: 5.1, modulationIndex: 32, resonance: 3000, octaves: 1.5, volume: -24,
    }).toDestination()
    ...
    const hat = new Tone.MetalSynth({
      ...
      harmonicity: 5.1, modulationIndex: 32, resonance: 3500, octaves: 1.5, volume: -26,
    }).toDestination()
